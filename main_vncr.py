...
def train(train_loader, model, model2, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        # virtual negative samples
        permuted_indices = torch.randperm(images[0].size(0))
        neg_images0 = partial_mixup(input=images[0], gamma=0.5, indices=permuted_indices)
        neg_images1 = partial_mixup(input=images[1], gamma=0.5, indices=permuted_indices)
        ...
        loss = criterion(features, labels, target_labels=list(range((opt.target_task+1)*opt.cls_per_task)))
